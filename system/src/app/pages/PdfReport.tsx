...
export default function PdfReport() {
  ...
  useEffect(() => {
    (async () => {
      if (!projectId) return;
      try {
        await transitionWorkflow({ projectId, stepId: 'report-pdf', to: 'finalized', note: 'Viewed Report PDF' });
      } catch {
        // ignore
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="border-b bg-card px-4 py-3 flex items-center gap-3">
        <div className="text-sm font-semibold">Report export</div>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            setFinalizing(true);
            try {
              const res = await finalizeDocument({ projectId, docType: 'report', note: 'Finalize Report export' });
              setFinalized(res);
              setVerification(null);
              setSigned(null);
              setChain(null);
              try {
                const ads = await listAddendums({ projectId, docType: 'report', releaseArtifactId: res.artifactId });
                setAddendums(ads);
              } catch {
                setAddendums([]);
              }
