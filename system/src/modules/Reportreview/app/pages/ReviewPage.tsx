...
export default function ReviewPage() {
  ...
  const canApprove = !hasUnacceptedBlockers && !hasUnacceptedWarnings;
  
  console.log('Approval status check:', {
    totalFindings: findings.length,
    acceptedFindings: findings.filter(f => f.acceptedRisk).length,
    hasUnacceptedBlockers,
    hasUnacceptedWarnings,
    canApprove,
    findings: findings.map(f => ({ id: f.id, severity: f.severity, acceptedRisk: f.acceptedRisk }))
  });
